...
| 🧰 通用采集 | 打开即自动监控 ComfyUI / Ollama / OpenCode / Kimi 桌面版 / PyCharm / Windsurf 等 20+ 工具（采集器内嵌，无需单独启动）|
| 📱 手机副屏 | 手机扫码配对后变成状态屏：像素小人 + 实时事件时间线，支持 OLED 纯黑常驻展示 |
| 📨 消息推送 | 任务完成 / 失败 / 等待授权时，推送到微信（Server酱 / PushPlus）或邮箱，人不在电脑前也能及时知道 |
| 🔊 语音播报 | 任务开始 / 进行 / 完成分段播报；本地神经网络音色优先（可靠、重启不丢失），可按软件单独配音色音量；另有「在线 HD」开关 |
| 📌 托盘常驻 | 后台托盘图标，右键「打开软件页面」/「退出软件」；干净退出不留残留进程 |
...
监控是自动的：程序启动后即监听 `~/.claude/projects/` 与 `~/.codex/sessions/` 的会话日志，Claude Code / Codex 一干活就能看到状态；同时内嵌采集器自动监控 ComfyUI / Ollama / OpenCode / Kimi 桌面版 / PyCharm 等工具。想要更细的事件，可另外双击 `安装ClaudeCode监控.cmd` / `安装Codex监控.cmd` 安装 hooks，并用 `安装VSCode扩展.cmd` 接入编辑器层监控。
